...
const NPTR = () => {
  const [dataNPTR, setDataNPTR] = useState([]);
  const [previewImage, setPreviewImage] = useState(null);

  const fetchDataNPTR = async () => {
    try {
      const res = await axios.get("https://halcyonone-internal-test.onrender.com/getNPTR");
      if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`https://halcyonone-internal-test.onrender.com/delete${id}`);
      if (res.data.success) {
  ...
  return (
    <div className="container mx-auto max-w-[1920px] h-dvh bg-[#F8F8FF] relative">
      <Navbar />
      <Searchbar fetchDataNPTR={fetchDataNPTR} />
